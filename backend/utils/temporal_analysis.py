...
class TemporalAnalysisEngine:
    ...
    VITALS_BASELINE = {
        ...
        'blood_pressure_systolic': {'min': 90.0, 'max': 120.0, 'ideal': 115.0, 'weight': 0.20},
        'blood_pressure_diastolic': {'min': 60.0, 'max': 80.0, 'ideal': 75.0, 'weight': 0.15},
        'blood_glucose': {'min': 70.0, 'max': 100.0, 'ideal': 85.0, 'weight': 0.25},
        'temperature': {'min': 36.1, 'max': 37.2, 'ideal': 36.6, 'weight': 0.25}
    }

    @classmethod
    def analyze_vitals(cls, 
        ...
        if heart_rate is not None:
            ...
            if heart_rate > hr_cfg['max']:
                ...
                penalties += hr_cfg['weight'] * severity
            analysis['metrics']['heart_rate'] = {'value': heart_rate, 'status': 'Normal' if (hr_cfg['min'] <= heart_rate <= hr_cfg['max']) else 'Abnormal'}
        ...
        if bp_systolic is not None and bp_diastolic is not None:
            ...
            if bp_systolic > 140 or bp_diastolic > 90:
                ...
                bp_severity = "warning"
                bp_msg = f"Low blood pressure ({int(bp_systolic)}/{int(bp_diastolic)} mmHg)."
        ...
        if blood_glucose is not None:
            ...
            if blood_glucose > glu_cfg['max']:
                ...
                penalties += glu_cfg['weight'] * severity
            analysis['metrics']['blood_glucose'] = {'value': blood_glucose, 'status': 'Normal' if (glu_cfg['min'] <= blood_glucose <= glu_cfg['max']) else 'Abnormal'}
        ...
        if temperature is not None:
            ...
            if temperature > temp_cfg['max']:
                ...
                penalties += temp_cfg['weight'] * severity
            analysis['metrics']['temperature'] = {'value': temperature, 'status': 'Normal' if (temp_cfg['min'] <= temperature <= temp_cfg['max']) else 'Abnormal'}
        ...
        if len(analysis['flags']) > 0:
            critical_flags = sum(1 for f in analysis['flags'] if f['severity'] == 'danger')
            if critical_flags > 0:
                analysis['summary'] = f"Warning: {critical_flags} critical vital anomaly detected! Medical assessment recommended."
            else:
                analysis['summary'] = f"Alert: {len(analysis['flags'])} minor vital deviations observed. Keep tracking."
                
        return analysis

    @classmethod
    def calculate_dynamic_survival(cls, 
                                   disease_posterior: float, 
                                   vitals_health_score: float, 
                                   trend_factor: float = 0.0) -> float:
        """
    ...
    def analyze_history_trends(cls, history: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for idx, record in enumerate(sorted_history):
            ...
            try:
                date_formatted = datetime.fromisoformat(date_str).strftime('%Y-%m-%d %H:%M')
            except Exception:
            ...
            posterior = record.get('bayesian_posterior') or (record.get('ml_probability') or 0.0)
            ...
            timeline.append({
                'id': record.get('id'),
                'date': date_formatted,
                'disease': record.get('disease', 'Unknown').replace('_', ' ').title(),
                'ml_probability': round((record.get('ml_probability') or 0.0) * 100, 1),
                'bayesian_posterior': round(posterior * 100, 1) if record.get('bayesian_posterior') else None,
                ...
                'health_index': round(vitals_analysis['vitals_health_score'] * 100, 1),
                'vitals': {
                    'heart_rate': record.get('heart_rate'),
                    'blood_pressure': f"{record.get('blood_pressure_systolic')}/{record.get('blood_pressure_diastolic')}" if record.get('blood_pressure_systolic') else None,
                    'blood_glucose': record.get('blood_glucose'),
                    'temperature': record.get('temperature')
                }
            })
            ...
            vitals_trends['ml_probability'].append(round((record.get('ml_probability') or 0.0) * 100, 1))
        ...
        survival_probability_fractions = [float(v) / 100.0 for v in vitals_trends['survival_probability']]
        
        # disease_history
        disease_history = [t['disease'] for t in timeline]
        
        # latest health score (0.0 to 1.0)
        latest_vitals_score = float(timeline[-1]['health_index']) / 100.0 if timeline else 1.0
        ...
        return {
            ...
            'latest_survival_probability': timeline[-1]['survival_probability'] if timeline else 100.0,
