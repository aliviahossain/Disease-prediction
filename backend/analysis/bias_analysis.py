...
class BiasAnalyzer:
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "bias_range": [
                round(float(np.min(biases)), 2),
                round(float(np.max(biases)), 2),
            ],
        }
    ...
    def _analyze_symptom_coverage(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "shared_symptoms": sum(
                1 for c in coverage.values() if c["disease_count"] > 1
            ),
            "unique_to_one_disease": sum(
                1 for c in coverage.values() if c["disease_count"] == 1
            ),
    ...
    def _simulate_per_disease_metrics(
        ...
        for disease in all_diseases:
            ...
            metrics[disease] = {
                ...
                "true_negatives": tn,
                "false_negatives": fn,
                "avg_positive_probability": round(
                    float(np.mean(probabilities_positive)), 3
                ),
                "avg_negative_probability": round(
                    float(np.mean(probabilities_negative)), 3
                ),
    ...
    def _compute_bias_indicators(self) -> Dict[str, Any]:
        ...
        cv = (
            float(np.std(symptom_counts) / np.mean(symptom_counts))
            if np.mean(symptom_counts) > 0
            else 0
        )
        ...
        return {
            ...
            "overall_balance_score": self._compute_balance_score(
                symptom_counts, biases
            ),
        }
    ...
    def _compute_balance_score(self, counts: List[int], biases: List[float]) -> Dict:
        ...
        return {
            ...
            "grade": (
                "A"
                if overall >= 80
                else "B" if overall >= 65 else "C" if overall >= 50 else "D"
            ),
        }
